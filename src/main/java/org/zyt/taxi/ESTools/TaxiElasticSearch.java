...
public class TaxiElasticSearch {
    ...
    private static int maxPathNum = 10000;
    private static int maxCarNum = 10000;
...
//        System.out.println(mapJson.string());
    ...
    public static List<String> searchIndexTaxiRoute(String startTime, String endTime, String carID,
        ...
        getClient();
        // 最大返回数
        int maxNum = maxPathNum;
        if (maxView != null && maxView != "") {
            maxNum = Integer.parseInt(maxView);
        }

        //时间范围的设定
        RangeQueryBuilder rangequerybuilder1 = QueryBuilders.rangeQuery("startTime");
        RangeQueryBuilder rangequerybuilder2 = QueryBuilders.rangeQuery("startTime");
        QueryBuilder qb = QueryBuilders.boolQuery();

        if(endTime != null && endTime != ""){
            rangequerybuilder1 = QueryBuilders
                    .rangeQuery("startTime")
                    .to(endTime);
            ((BoolQueryBuilder) qb).must(rangequerybuilder1);
        }
        if(startTime != null && startTime != ""){
            rangequerybuilder2 = QueryBuilders
                    .rangeQuery("endTime")
                    .from(startTime);
            ((BoolQueryBuilder) qb).must(rangequerybuilder2);
        }

//        if( != rangequerybuilder1 ){
//
//        }
//        // 构造查询对象
//        QueryBuilder qb = QueryBuilders.boolQuery()
...
//            System.out.println(hits.getHits()[i].getSourceAsString());
    ...
    public static String getCarNum() throws UnknownHostException {
        ...
        // terms聚合，获得carID个数
        AggregationBuilder teamAgg = AggregationBuilders.terms("carID_count").field("carID").size(maxPathNum);
        ...
        Stats s = response2.getAggregations().get("minDate_count");
        minmaxDate = "{\"min\":" + s.getMin() +", \"max\":" + s.getMax() +"}";
